    ...
    for x in range(sequence1.start, sequence1.end):
        for y in range(sequence2.start, sequence2.end):
            ...
            if is_pandigital_1_through_9(f"{x}{y}{product}"):
                sum_of_products_whose_multiplicand_multiplier_product_are_a_1_through_9_pandigital.add(
                    product)
    return sum_of_products_whose_multiplicand_multiplier_product_are_a_1_through_9_pandigital


def pandigital_products():
    sum_of_products_whose_multiplicand_multiplier_product_are_a_1_through_9_pandigital = get_products(
        sequence(1, 10000), sequence(1, 10))
    sum_of_products_whose_multiplicand_multiplier_product_are_a_1_through_9_pandigital.update(
        get_products(sequence(100, 1000), sequence(10, 100)))
